...
public class DeltaRule extends LearningAlgorithm {
    ...
    public DeltaRule(NeuralNet neuralNet, NeuralDataSet trainDataSet, LearningMode learningMode) {
        ...
        if (layer > 0)
            ...
            switch (learningMode) {
                case BATCH:
                    ...
                    for (int i = 0; i < trainingDataSet.getNumberOfRecords(); i++)
                        multDerivResultIthInput += error.get(i).get(neuron) * derivativeResult.get(i) * ithInput.get(i);
        ...
        if (layer > 0)
            ...
            switch (learningMode) {
                case BATCH:
                    ...
                    double multDerivResultIthInput = 0.0;
                    for (int i = 0; i < trainingDataSet.getNumberOfRecords(); i++)
                        multDerivResultIthInput += derivativeResult.get(i) * ithInput.get(i);
                    deltaWeight *= multDerivResultIthInput;
                    break;
                case ONLINE:
                    deltaWeight *= currNeuron.derivative(neuralNet.getInputs());
